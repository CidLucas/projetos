# Status — Rastro

> Última atualização: 2026-07-29
> **Contrato:** Deep Blue → Rastro (direto)
> **Investimento:** R$ 25.000 (2 meses, pagamento por entrega: 50% após Fase A + 50% após Fase B)
> **Produto:** Rastro Brain (MCP Brain Lite)
> **Lucas:** responsável técnico + consultor
> **Foco:** Escopo fechado — próximo passo é apresentar proposta à Rastro

## 🩺 Saúde geral

🟢 **Escopo fechado** — Todas as perguntas críticas respondidas. Documento de escopo (`00-escopo-proposta.md`) v0.3 pronto para virar proposta comercial.

## 📊 Resumo executivo

| Item | Status |
|---|---|
| **Escopo Fase A (consultoria)** | 🟢 ~3 semanas |
| **Entregáveis Fase A** | 🟢 Processo de propostas atualizado + Relatório do corpus documental |
| **Escopo Fase B (implementação)** | 🟢 ~5 semanas |
| **Critério de pronto Fase B** | 🟢 Time conectado + conteúdo curado e disponível via MCP |
| **Investimento** | 🟢 R$ 25.000 (50% após Fase A + 50% após Fase B) |
| **Produto** | 🟢 Rastro Brain (MCP Brain Lite) |
| **Escopos** | 🟢 Pessoal + Corporativo |
| **Curadoria** | 🟢 Fábio e Lucas Diárea (Rastro) |
| **Deploy** | 🟡 Provável EC2 (a confirmar) |
| **Case/Portfolio** | 🟢 Rastro autoriza mencionar |
| **Brain Lite testado** | 🔴 Não — Plano B: conector MCP simples direto no Rastro Mind |
| **Proposta comercial** | 🟢 Documento de escopo pronto |
| **Contrato** | 🔴 Aguardando aprovação da proposta |
| **Data de kickoff** | 🔴 A definir |

## 🚧 Blockers / Riscos

- **Risco #1 (técnico):** MCP Brain Lite não testado em produção. Plano B: conector MCP simples direto no Rastro Mind.
- **Risco #2 (comercial):** proposta ainda não foi apresentada à Rastro.

## 🎯 Próximas ações

- [ ] **Lucas** — apresentar proposta à Rastro
- [ ] **Lucas** — testar tools MCP do Brain Lite (em paralelo)
- [ ] **Hermes** — manter docs atualizados

## ❓ Perguntas em aberto

Nenhuma pergunta crítica. Todas as definições foram fechadas em 2026-07-29.

## 📅 Histórico de atualizações

| Data | Atualização |
|---|---|
| 2026-07-29 | Criação do projeto. Site da Rastro + Rastro Mind analisados. Docs base criados. |
| 2026-07-29 | v0.2: Cronograma 2 meses. Investimento R$ 25.000. Docs 01/02/04 removidos. |
| 2026-07-29 | v0.3: Perguntas fechadas. Entregáveis detalhados. Produto nomeado Rastro Brain. Curadoria: Fábio + Lucas Diárea. Deploy: EC2. Case autorizado. |
